...
function createHelpers(pool, query, TENANT, SETTING_DEFAULTS, SETTING_TYPES) {
  ...
  /** Parse the service_hours_overrides JSON setting: { "<ourDay>": {start, end} }.
   *  Malformed input degrades to no overrides. */
  function parseHourOverrides(raw) {
    if (!raw) return {};
    try {
      ...
      for (const [day, w] of Object.entries(obj)) {
        const d = Number(day);
        if (d >= 0 && d <= 6 && w && timeRe.test(w.start) && timeRe.test(w.end) && w.start !== w.end) {
          clean[d] = { start: w.start, end: w.end };
        }
      }
      return clean;
    } catch {
  ...
  /** Per-day service window resolution: override for the day, else global. */
  async function getServiceWindows() {
    const startStr = await getSetting('service_hours_start', '08:00');
    const endStr = await getSetting('service_hours_end', '19:00');
    const overrides = parseHourOverrides(await getSetting('service_hours_overrides', ''));
    return {
      hoursFor(ourDay) {
        return overrides[ourDay] || { start: startStr, end: endStr };
      },
      overrides,
      defaults: { start: startStr, end: endStr }
    };
  }
  ...
  async function isWithinServiceHours(requestedTime, timezone) {
    ...
    const t = p.h * 60 + p.min;

    // Same calendar day's window (an overnight window's evening segment).
    const today = windows.hoursFor(ourDay);
    if (opDays.includes(ourDay) && windowSegment(t, today.start, today.end) === 'same-day') return true;

    // Early morning may be the tail of the PREVIOUS day's overnight window
    // (e.g. Saturday 01:30 during Friday 22:00-03:00 service).
    const prevDay = (ourDay + 6) % 7;
    const prev = windows.hoursFor(prevDay);
    if (opDays.includes(prevDay) && isOvernightWindow(prev.start, prev.end)) {
      const [eH, eM] = prev.end.split(':').map(Number);
      if (t <= eH * 60 + (eM || 0)) return true;
    }
    return false;
  }
  ...
  async function getServiceHoursMessage() {
    ...
    const windows = await getServiceWindows();
    if (Object.keys(windows.overrides).length) {
      const schedule = opDays.map(d => `${dayLabels[d]} ${windows.hoursFor(d).start}-${windows.hoursFor(d).end}`).join(', ');
      return `Requested time outside service hours (${schedule})`;
    }
    const dayNames = opDays.map(d => dayLabels[d]).join(', ');
    return `Requested time outside service hours (${windows.defaults.start}-${windows.defaults.end} ${dayNames})`;
  }
