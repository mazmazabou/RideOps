...
const DEFAULT_TENANT = {
  ...
  orgInitials: 'RO',
  // Single-tenant deployments MUST set TENANT_TIMEZONE (or TENANT_FILE) in
  // production — the host machine's clock (often UTC on PaaS) is only a dev
  // fallback. Campus-scoped requests resolve their own timezone regardless.
  timezone: process.env.TENANT_TIMEZONE || Intl.DateTimeFormat().resolvedOptions().timeZone,
  primaryColor: '#4682B4',
...
const VALID_ORG_SLUGS = ['usc', 'stanford', 'ucla', 'uci', 'scranton'];
...
const SETTING_TYPES = {
  ...
  service_hours_overrides: 'string',
  operating_days: 'string',
  auto_deny_outside_hours: 'boolean',
  service_closed: 'boolean',
  service_closed_message: 'string',
