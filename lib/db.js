...
function createDb(pool, deps) {
  ...
  async function runMigrations() {
    const statements = [
      ...
      // Party size: how many riders board together (vans seat multiple)
      `ALTER TABLE rides ADD COLUMN IF NOT EXISTS party_size INTEGER NOT NULL DEFAULT 1;`,

      // Student-facing notices (separate from internal program rules)
      `ALTER TABLE program_content ADD COLUMN IF NOT EXISTS student_rules_html TEXT NOT NULL DEFAULT '';`
    ];
  ...
  async function seedDefaultSettings() {
    const defaults = [
      ...
      { key: 'operating_days', value: '0,1,2,3,4,5,6', type: 'string', label: 'Operating Days', description: 'Days of the week service operates (0=Mon, 1=Tue, ... 6=Sun)', category: 'operations' },
      { key: 'service_hours_overrides', value: '', type: 'string', label: 'Per-Day Service Hours', description: 'JSON of per-day hour overrides keyed by day (0=Mon..6=Sun), e.g. {"4":{"start":"22:00","end":"03:00"}}. Days without an override use the global start/end.', category: 'operations' },
      { key: 'service_closed', value: 'false', type: 'boolean', label: 'Service Temporarily Closed', description: 'When on, riders see the closure message and new ride requests are declined (weather, breakdowns, early shutdown).', category: 'operations' },
      { key: 'service_closed_message', value: '', type: 'string', label: 'Closure Message', description: 'Message shown to riders while the service is closed.', category: 'operations' },
